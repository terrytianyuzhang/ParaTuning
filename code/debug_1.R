#####why the training errors are of different scales
...
save(risk.score.list, file = '/raid6/Tianyu/PRS/CombinedLassoSum/Tmp/debug1-riskscore.Rdata')

#####

###########generate Y##############
risk.score.list <- get(load('/raid6/Tianyu/PRS/CombinedLassoSum/Tmp/debug1-riskscore.Rdata'))
bootys <- vector("list", 2)
...
a <- as.numeric(a)
a <- a/2
print(paste0('the a used for CEU is ',a))
...
a <- as.numeric(a)
a <- a/4
print(paste0('the a used for YRI is ',a))
...
#######
###generate the CV summary stat
if(1){
  setting.title <- 'debug1CV'
  nfold <- 4000
...
####generate the summary stat for complete data
if(1){
