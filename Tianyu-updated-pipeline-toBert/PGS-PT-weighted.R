...
fwrite(df.train,paste0(work.dir,"weighted-PT.r2_p"),row.names=F,col.names=T,quote=F,sep="\t")
...
for(tune.set in tune.sets){
  ...
  for(train.set in train.sets){
    ...
    PRS[[train.set]]$SCORE1_AVG=as.vector(scale(PRS[[train.set]]$SCORE1_AVG))
  }
  ...
  AUC.weighted=NULL
  for(wght in wghts){
    AUC.weighted=rbind.data.frame(AUC.weighted,data.frame(wght=wght,AUC=auc(PRS[[train.sets[1]]]$PHENO1-1~eval(wght*PRS[[train.sets[1]]]$SCORE1_AVG+(1-wght)*PRS[[train.sets[2]]]$SCORE1_AVG))))
  }
  
  plot(AUC.weighted$wght,AUC.weighted$AUC,main=tune.set)
  save(AUC.weighted, 
       file = paste0(tune.dir,"PRS/","weightedPT-AUC.RData"))
...
fwrite(re.wght,paste0(work.dir,"weighted-PT.wghts"),row.names=F,col.names=T,quote=F,sep="\t")

##################################################################################
#### now for the validation set calculate the PRS based on df.train and max.wght
re.p_r2=fread(paste0(work.dir,"weighted-PT.r2_p"),header=T,data.table=F)
re.wght=fread(paste0(work.dir,"weighted-PT.wghts"),header=T,data.table=F)
...
for(test.set in test.sets){
  ...
  WEIGHTED.PRS=cbind(PRS$CEU.TRN[,c(1,2,3,6)],PRS$YRI.TRN[,6])
  colnames(WEIGHTED.PRS)[(ncol(WEIGHTED.PRS)-1):ncol(WEIGHTED.PRS)]=paste0(train.sets,".scaled")
  wght=re.wght[re.wght$anc == substr(test.set,1,3),"wght"]
  WEIGHTED.PRS$WEIGHTED.scaled=as.vector(scale(wght*PRS[[train.sets[1]]]$SCORE1_AVG+(1-wght)*PRS[[train.sets[2]]]$SCORE1_AVG))
  fwrite(WEIGHTED.PRS,paste0(test.dir,"PRS/",test.set,"-weighted-PGS.txt"),row.names=F,col.names=T,quote=F,sep="\t")
  (auc(WEIGHTED.PRS$PHENO1-1~WEIGHTED.PRS$CEU.TRN.scaled)[1])
  (auc(WEIGHTED.PRS$PHENO1-1~WEIGHTED.PRS$YRI.TRN.scaled)[1])
  (auc(WEIGHTED.PRS$PHENO1-1~WEIGHTED.PRS$WEIGHTED.scaled)[1])
