...
PGS_bychr_bootstrap <- function(chr, anc, beta, work.dir){
  ...
  system.time(gnt<-read.plink(bed=paste0(work.dir, "TST/CHR/",anc,".TST-chr",chr,".bed"),
                              bim=paste0(work.dir, "TST/CHR/",anc,".TST-chr",chr,".bim"),
                              fam=paste0(work.dir, "TST/CHR/",anc,".TST-chr",chr,".fam"),
                              select.snps = snp)
  )
...
for(gamma in GAMMA){
  ...
  for(i.set in 1:2){
    ...
    pheno <- fread(file = paste0(work.dir, "TST/CHR/",anc,".TST-chr22.fam"))
