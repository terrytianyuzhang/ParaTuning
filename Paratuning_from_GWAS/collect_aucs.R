...
all_gammaGenerateData <- c(0.8,0.5,0.2,0.8)
all_lambdaIndexGenerateData <- c(5,5,8,1)

ancestries <- c("CEU", "YRI")

####

for(synthetic_method in 1:length(all_gammaGenerateData)){
  gammaGenerateData <- all_gammaGenerateData[synthetic_method]
  lambdaIndexGenerateData <- all_lambdaIndexGenerateData[synthetic_method]
  ...
  for(simulation_index in as.character(600:609)){
    
    work.dir <- paste0("/raid6/Tianyu/PRS/parameter_tuning_from_GWAS/Sim-", simulation_index, "/") #"/raid6/Tianyu/PRS/SimulationPipeline/Work/Sim-800/"
    
    jointlassosum_parameter_tuning_directory <- paste0(work.dir, "ParameterTuningData",
    ...
    jointlassosum_directory <- paste0(work.dir, "JointLassoSum/")
  ...
  parameter_tuning_result_directory <- paste0("/raid6/Tianyu/PRS/parameter_tuning_from_GWAS/Data/")
