...
# set parameters, this is the grid used to find the best lasso parameters
...
for(train.set in train.sets){
  ...
  for(tune.set in tune.sets){
    test.set=gsub("TUNE","TST",tune.set)
    ...
    for(chr in 1:22){
      ...
      timestamp()
      cl <- makeCluster(32, type="FORK") # Parallel over 10 nodes
