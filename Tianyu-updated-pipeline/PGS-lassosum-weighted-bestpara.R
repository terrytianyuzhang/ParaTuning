...
for(test.set in test.sets){
  ...
  AUC.weighted=NULL
  for(wght in wghts){
  ####calculate AUC for each weight
    AUC.weighted=rbind.data.frame(AUC.weighted,data.frame(wght=wght,
                                                          AUC=auc(PRS[[train.sets[1]]]$pheno~eval(wght*PRS[[train.sets[1]]]$best.pgs+(1-wght)*PRS[[train.sets[2]]]$best.pgs))))
  }
  
  save(AUC.weighted, file = paste0(test.dir,"Lassosum/",test.set,"/WEIGHTED-",test.set,"-lassosum.AUC.RData"))
