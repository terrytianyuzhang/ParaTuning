...
source("R-code/LS-pipeline-functions.R")
...
####### JOINT LASSOSUM
# functions needed
source("R-code/jls-functions.R")  
Rcpp::sourceCpp(paste0("R-code/myfunctions.cpp"))
...
for(sim in c(701:709,800:809)){
  ...
  re<-jlsSelectSnp(TRN1="CEU",TRN2="YRI", REF1="CEU", REF2="YRI", WORK.DIR = paste0(work.dir,"/",sim),REF.DIR = ref.dir,
                  ld.populations = ld.populations, N = N,shrink = c(0.7,0.8,0.9),lambda=sort(0.021-exp(seq(log(0.001), log(0.02), length.out=20)))[4:8], 
                  gamma=seq(0.76,0.86,.02),chrs=1:22)
}
...
for(sim in c(701:709,800:809)){
  ...
  re.JLS.TUNING=NULL
  for(TUNE in c("CEU","YRI")){
    re.jlsTuning<-jlsTuning(TRN1 = "CEU", TRN2 = "YRI", REF1= "CEU", REF2 = "YRI", TUNE = TUNE, WORK.DIR = paste0(work.dir,"/",sim), plink2 = plink2)
